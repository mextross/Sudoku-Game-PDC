...
//The panel that queries and draws the corresponding easy pattern chosen by the user
public class EasyPatternPanel extends PatternPanel implements ActionListener {
    ...
    private DBEasyPatternOperations query;
    private ArrayList<int[]> easyPattern;
    private ArrayList<int[]> solvedEasyPattern;
    private ArrayList<int[]> solvedArrayValues;
    private int[] easyPatternAttributes;
    private int[] solvedEasyPatternAttributes;
    ...
    public EasyPatternPanel() {
        ...
        levelTitle.setText("Let's play! Please fill in the blanks with numbers 1 to 3!");
        ...
        //Middle Panel
        query = new DBEasyPatternOperations();
        levelPatternChoicePanel = new JPanel();
        levelPatternChoicePanel.setLayout(new GridLayout(3, 3));
        easyPatternAttributes = new int[4];
        solvedEasyPatternAttributes = new int[4];
        easyPattern = new ArrayList<>();
        solvedEasyPattern = new ArrayList<>();
    ...
    public void queryTableForPattern(int patternNum) throws SQLException {
        ...
        ResultSet rs = query.getEasyPattern(patternNum);
        while (rs.next()) {
            ...
            int y = rs.getInt(4);
            easyPatternAttributes = new int[4];
            easyPatternAttributes[0] = rowID;
            easyPatternAttributes[1] = element;
            easyPatternAttributes[2] = x;
            easyPatternAttributes[3] = y;
            easyPattern.add(easyPatternAttributes);
        }
    }
    ...
    public void queryTableForSolvedPattern(int patternNum) throws SQLException {
        checkWithinBounds(patternNum, "patternNum");
        solvedEasyPattern.clear();
        query.establishConnection();
        ResultSet rs = query.getSolvedEasyPattern(patternNum);
        while (rs.next()) {
            int rowID = rs.getInt(1);
            int element = rs.getInt(2);
            int x = rs.getInt(3);
            int y = rs.getInt(4);
            solvedEasyPatternAttributes = new int[4];
            solvedEasyPatternAttributes[0] = rowID;
            solvedEasyPatternAttributes[1] = element;
            solvedEasyPatternAttributes[2] = x;
            solvedEasyPatternAttributes[3] = y;
            solvedEasyPattern.add(solvedEasyPatternAttributes);
        }
    }
    ...
    public void drawPatternPanel() throws SQLException, ParseException {
        ...
        componentList.clear();
        puzzleIndexes.clear();
        for (int[] e : easyPattern) {
            if (e[1] == 0) {
                puzzleIndexes.add(easyPattern.indexOf(e));
    ...
    public void compareValues() {
        ...
        for (int i : puzzleIndexes) {
            int[] solvedArray = solvedEasyPattern.get(i);
    ...
    //a method that calls all other relevant methods to make an EasyPatternPanel
    public void makeEasyPatternPanel(int patternNum) throws SQLException, ParseException {
